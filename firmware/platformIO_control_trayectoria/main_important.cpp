...
void axis_calibrate_range(Axis &ax, int homing_search_limit_steps, int homing_min_position_steps, int homing_backoff_steps, int homing_speed_steps_per_sec){
  ...
  while (!button_pressed(limit_min_pin)) {
    axis_move_steps(ax, dir_to_min, homing_search_limit_steps, homing_speed_steps_per_sec);
  ...
  while (!button_pressed(limit_max_pin)) {
    axis_move_steps(ax, dir_to_max, homing_search_limit_steps, homing_speed_steps_per_sec);
    while (ax.steps_remaining) {vTaskDelay(pdMS_TO_TICKS(100));}
  }
...
void run_calibration(){
  // Axis Y calibration
  axis_calibrate_range(axis_y, HOMING_SEARCH_LIMIT_Y_STEPS, HOMING_MIN_POSITION_Y_STEPS, HOMING_BACKOFF_Y_STEPS, HOMING_SPEED_Y_STEPS_PER_SEC);
  // Axis X calibration
  axis_calibrate_range(axis_x, HOMING_SEARCH_LIMIT_X_STEPS, HOMING_MIN_POSITION_X_STEPS, HOMING_BACKOFF_X_STEPS, HOMING_SPEED_X_STEPS_PER_SEC);
  // Axis Knee calibration
  axis_calibrate_range(axis_k, HOMING_SEARCH_LIMIT_K_STEPS, HOMING_MIN_POSITION_K_STEPS, HOMING_BACKOFF_K_STEPS, HOMING_SPEED_K_STEPS_PER_SEC);
}
