...
from src.ui.screens.variability_matrix_screen import VariabilityMatrixScreen
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, serial_port: str, theme_manager: ThemeManager, parent=None):
        ...
        # Height-variability/repeatability test matrix (2026-09-18) —
        # replaced the old flat "Pruebas" screen (tara_history_screen.py)
        # per Luis's explicit choice: results now organized by
        # talla (column) x depth-below-tara (row) instead of an
        # undifferentiated list. Needs the bridge (unlike the old
        # read-only screen) since executing a matrix point is a real
        # blocking device action.
        self._variability_matrix_screen = VariabilityMatrixScreen(self._bridge, theme_manager)

        self._stack = QStackedWidget()
        self._stack.addWidget(self._connection_screen)         # index 0
        self._stack.addWidget(self._trajectory_screen)          # index 1
        self._stack.addWidget(self._variability_matrix_screen)  # index 2
    ...
    def _build_nav_bar(self) -> QWidget:
        ...
        self._variability_matrix_nav_btn = QPushButton("Matriz de Pruebas")
        self._variability_matrix_nav_btn.setCheckable(True)
        self._variability_matrix_nav_btn.setStyleSheet(NAV_BUTTON_STYLE())
        self._variability_matrix_nav_btn.clicked.connect(lambda: self._stack.setCurrentIndex(2))

        self._nav_group.addButton(self._connection_nav_btn)
        self._nav_group.addButton(self._trajectory_nav_btn)
        self._nav_group.addButton(self._variability_matrix_nav_btn)

        layout.addWidget(self._connection_nav_btn)
        layout.addWidget(self._trajectory_nav_btn)
        layout.addWidget(self._variability_matrix_nav_btn)
        layout.addStretch()
    ...
    def _apply_theme(self, theme_name: str) -> None:
        ...
        for button in (
            ...
            self._variability_matrix_nav_btn, self._calibration_map_btn,
        ):
