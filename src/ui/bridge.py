...
class StateMachineBridge(QObject):
    ...
    disconnected = Signal()
    connected = Signal()
    # axis ("Y"/"X"/"A"), bound ("MIN"/"MAX"), value (float or None for MIN)
    calibration_limit = Signal(str, str, object)
    calibration_progress = Signal(str, float)  # axis, value
    ...
    def __init__(self, state_machine: SystemStateMachine, parent=None):
        ...
        self._state_machine.on_calibration_progress = self._on_calibration_progress
    ...
    def _on_calibration_progress(self, axis: str, value: float) -> None:
        self.calibration_progress.emit(axis, value)